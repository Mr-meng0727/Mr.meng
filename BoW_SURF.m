...
numberOfClusters = 800; % The number of clusters representing the number of features in the bag of features. Default can be 500.
...
for i = 1:length(classes)
    imgSets = [ imgSets, imageSet(fullfile('TrainData\', classes{i})) ];
end
...
testSet = [];
for i = 1:length(classes)
    testSet = [ testSet, imageSet(fullfile('TrainData\', classes{i})) ];
end

% testSet = imageSet('\TestData\', 'recursive');
% randomSet = testSet(2);
% testSet(2) = [];
